...
static uint8_t EEPROM_Utils::saveScore(char *name) {
      ...
      for (uint8_t j = i; j > 0; j--) {
        ...
        if (getHighScore(EEPROM_HS_SCORE_1 + (j * 2)) >= getHighScore(EEPROM_HS_SCORE_1 + ((j -1 ) * 2))) {
          ...
          for (uint8_t k = 0; k < NAME_LENGTH + 1; k++) {
            ...
            EEPROM.update(EEPROM_HS_NAME_1 + (j * 10) + k, b);
            EEPROM.update(EEPROM_HS_NAME_1 + ((j - 1) * 10) + k, a);
  ...
  if (idx < NO_WINNER) {
    ...
    for (uint8_t j = 0; j < NAME_LENGTH; j++) {
      ...
      EEPROM.update(EEPROM_HS_NAME_1 + (idx * 10) + j, name[j]);
